...
const int kMaxUndo = 12;//changed from float to int. jiangshan
const float kPaletteErrorTolerance = 1.0f;//3.0f;
const double kSubclusterTolerance = 1.6f;
...
class Pix {
 ...
 public:
  ...
  void Iterate(int is_print=0, int is_save_dbg_img=0);
  ...
  inline void GetInputImage(cv::Mat& img) { cv::cvtColor(input_img_, img, cv::COLOR_Lab2RGB);}

  //returns an 8U, rgb image representing the superpixel color values
  inline void GetSuperpixelImage(cv::Mat& img) {
    cv::Mat rgb;
    cv::cvtColor(GetCurrentState()->superpixel_color, rgb, cv::COLOR_Lab2BGR);
  ...
  int set_debug_filename(std::string f)
  {
      debug_filename_prefix= f;
      return 0;
  }
...
private:
  ...
  // get current palette image
  int GetPaletteImage(cv::Mat &pal, int one_pal_w=64, int one_pal_h=64 );

  //returns the current algorithm state
  pixState s_state;
  //inline pixState * GetCurrentState(){return state_list_->getCur();}
  inline pixState * GetCurrentState(){return &s_state;}
  ...
  cv::Mat m_region_map;
  cv::Mat m_slic_error;//old value is named distance
