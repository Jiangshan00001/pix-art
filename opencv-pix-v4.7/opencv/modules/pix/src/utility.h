...
namespace pix_research
{
  ...
  {
    return (float)(exp((x-mean)*(x-mean)/(-2.0f*sigma*sigma))/sqrt(6.28319*sigma*sigma));
  }
  ...
  inline cv::Vec3f rgb2lab(cv::Vec3f rgb)
  {
    ...
    cv::Mat holder2;
    cvtColor(holder, holder2,cv::COLOR_RGB2Lab);
  ...
  inline cv::Vec3f lab2rgb(cv::Vec3f lab)
  {
    ...
    cv::Mat holder2;
    cvtColor(holder, holder2,cv::COLOR_Lab2RGB);
    return holder2.at<cv::Vec3f>(0,0);
  }
  inline cv::Vec3f lab2bgr(cv::Vec3f lab)
  {
    cv::Mat holder(cv::Size(1,1),CV_32FC3);
    holder.at<cv::Vec3f>(0,0) = lab;
    cv::Mat holder2;
    cvtColor(holder, holder2,cv::COLOR_Lab2BGR);
    return holder2.at<cv::Vec3f>(0,0);
  }
  inline std::string num2str(int k)
  {
