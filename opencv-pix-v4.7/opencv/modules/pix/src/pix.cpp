...
Pix::Pix(const cv::Mat& img_input, int w, int h, int p)
{
  ...
  sigma_color_	= 4.5f;///.87f;
  sigma_position_ = .87f;
  state_list_ = new stateList(kMaxUndo);
  converged_flag_ = false;
  palette_maxed_flag_ = false;
  GetCurrentState()->saturation = 1.1f;

   //std::cout<<"img_1:"<<img_input<<"\n";
  //in8U.convertTo(in32, CV_32FC3, 1.0/255.0);
  img_input.convertTo(input_img_, CV_32FC3, 1.0/255.0);

  //std::cout<<"img_2:"<<input_img_<<"\n";
  cvtColor(input_img_, input_img_, cv::COLOR_BGR2Lab);
...
Pix::Pix(std::string filename)
{
  ...
  range_ = (int)( sqrt((input_height_/(float)output_height_) *
...
void Pix::Initialize()
{
  ...
  m_region_map = cv::Mat(cv::Size(input_width_, input_height_),CV_32SC2);
  m_slic_error = cv::Mat(cv::Size(input_width_, input_height_), CV_32FC1, cv::Scalar(-5.0f));
  ...
  for(int y = 0; y < input_height_; ++y) {
    for(int x = 0; x < input_width_; ++x) {
      ...
      m_region_map.at<cv::Vec2i>(y,x) = cv::Vec2i(i,j);
  ...
  GetCurrentState()->prob_c.push_back(.5f);
  prob_co_.push_back(std::vector<float>(output_width_*output_height_,.5f));
  prob_co_.push_back(std::vector<float>(output_width_*output_height_,.5f));
  ...
  GetCurrentState()->locked_colors = 
    std::vector<bool>(2*max_palette_size_, false);
  GetCurrentState()->pixel_constraints = 
    std::vector<std::list<int> >(output_width_*output_height_);
...
void Pix::SaveDebugImages()
{

        ////debug for tmp file
        ///
        cv::Mat reg, sup, out, pal;
        std::ostringstream convert2;
        convert2 << this->iterate_cnt;
        std::string iterate_cnt_str  = convert2.str(); //int to string, Text="456"
        std::ostringstream pal_convert;
        int num_colors = (int)GetCurrentState()->palette.size();

        pal_convert<<num_colors;
        std::string palette_size_str = pal_convert.str();

        this->GetRegionImage(reg);
        this->GetSuperpixelImage(sup);
        this->GetOutputImage(out);
        this->GetPaletteImage(pal);
        cv::imwrite(this->debug_filename_prefix+"_debug_I"+iterate_cnt_str+"_P"+palette_size_str+"_region.bmp", reg );
        cv::imwrite(this->debug_filename_prefix+"_debug_I"+iterate_cnt_str+"_P"+ palette_size_str+"_super.bmp",  sup );
        cv::imwrite(this->debug_filename_prefix+"_debug_I"+iterate_cnt_str+"_P"+   palette_size_str+"_out.bmp",    out );
        cv::imwrite(this->debug_filename_prefix+"_debug_I"+iterate_cnt_str+"_P"+   palette_size_str+"_pal.bmp",    pal );
}

void Pix::Iterate(int is_print, int is_save_dbg_img)
{
  ...
  //std::cout << "UpdateSuperpixelMapping took "<< duration << "seconds" << std::endl;

  UpdateSuperpixelMeans();

  duration = ( std::clock() - start ) / (double) CLOCKS_PER_SEC;
  //std::cout << "UpdateSuperpixelMeans took "<< duration << "seconds" << std::endl;

  //update palette
  AssociatePalette();

  duration = ( std::clock() - start ) / (double) CLOCKS_PER_SEC;
  //std::cout << "AssociatePalette took "<< duration << "seconds" << std::endl;

  float err_pal = RefinePalette();

  duration = ( std::clock() - start ) / (double) CLOCKS_PER_SEC;
  //std::cout << "RefinePalette took "<< duration << "seconds" << std::endl;
  ...
  if(err_pal < kPaletteErrorTolerance)
  {
    ...
    int is_expand = ExpandPalette();
    //if (is_expand && is_print)
    //{
    //    printf("iterate_cnt=%d. expand palette finish. added=%d\n", this->iterate_cnt, is_expand);
    //    SaveDebugImages();
    //}
  }
   ...
   {
    int num_colors = (int)GetCurrentState()->palette.size();
    printf("err_pal=%f. temperature=%f. palette_size=%d\n", err_pal, temperature_, num_colors);
   }
...
{
  int current_palette_size = (int)GetCurrentState()->palette.size();
  ...
  for(int y = 0; y<output_height_; ++y) {
    for(int x = 0; x<output_width_; ++x) {
      ...
      for(std::list<int>::iterator nCol = constraints.begin(); nCol != constraints.end(); ++nCol) {
        ...
        prob_co_[color_index][vec2idx(cv::Vec2i(x,y))] = (float)normalized_prob;
        new_prob_c[color_index] += (float)( prob_sp*normalized_prob);
        constraints_index++;
      }
    }
  }
...
void draw_one_color(cv::Mat &dst, cv::Vec3f color, cv::Point p1, cv::Point p2, int index, float saturation)
{
    cv::Vec3f lab = color;
    lab[1] *= saturation;
    lab[2] *= saturation;
    cv::Vec3f bgr = lab2bgr(lab);
    bgr *=255.0;
    //std::cout<<"draw_one_color:"<<color<<"."<<bgr<<"\n";
    cv::rectangle(dst, p1,p2, bgr, 5);
    cv::putText(dst, pix_research::num2str(index),
                cv::Point((p1.x+p2.x)/2, (p1.y+p2.y)/2 ), cv::FONT_HERSHEY_SIMPLEX, 0.8,cv::Scalar(255, 255, 255),2  );

}

int Pix::GetPaletteImage(cv::Mat &pal, int one_pal_w, int one_pal_h)
{
    ///get a current palette image
    ///
    ///
    std::vector<cv::Vec3f> averaged_palette;
    averaged_palette = GetCurrentState()->palette;

    cv::Mat pal_tmp(cv::Size(3*one_pal_w,averaged_palette.size()*one_pal_h), CV_8UC3);
    pal_tmp=0;
    ...
    if (!palette_maxed_flag_)
    {
        for(int i = 0; i< GetCurrentState()->sub_superpixel_pairs.size(); ++i)
        {
            ...
            cv::Vec3f avg_color(weight_1*color_1[0]+weight_2*color_2[0],
              weight_1*color_1[1]+weight_2*color_2[1],
              weight_1*color_1[2]+weight_2*color_2[2]);

            draw_one_color(pal_tmp, color_1, cv::Point(0, i *one_pal_h+5 ),  cv::Point(one_pal_w-5, (i+1) *one_pal_h-5 ),index_1, GetCurrentState()->saturation);
            draw_one_color(pal_tmp, color_2, cv::Point(one_pal_w+5, i *one_pal_h+5 ),  cv::Point(one_pal_w*2-5, (i+1) *one_pal_h-5 ),index_2, GetCurrentState()->saturation);
            draw_one_color(pal_tmp, avg_color, cv::Point(one_pal_w*2+5, i *one_pal_h+5 ),  cv::Point(one_pal_w*3-5, (i+1) *one_pal_h-5 ),0, GetCurrentState()->saturation);

            //cv::Vec3f lab = color_1;
            //lab[1] *= GetCurrentState()->saturation;
            //lab[2] *= GetCurrentState()->saturation;
            //cv::Vec3f bgr = lab2rgb(lab);
            //cv::rectangle(pal_tmp, cv::Point(0, i *one_pal_h ),cv::Point(one_pal_w, (i+1) *one_pal_h ), bgr,5   );
            //cv::putText(pal_tmp,pix_research::num2str(index_1),cv::Point(one_pal_w/3, i *one_pal_h ), FONT_HERSHEY_SIMPLEX, 0.8,Scalar(255, 255, 255),2  );
        }
    }
    else
    {
        ...
        {
            cv::Vec3f color_1 = GetCurrentState()->palette[i];
            draw_one_color(pal_tmp, color_1, cv::Point(0, i *one_pal_h+5 ),  cv::Point(one_pal_w-5, (i+1) *one_pal_h-5 ),i, GetCurrentState()->saturation);
...
void Pix::GetOutputImage(cv::Mat& img) {
  ...
  cv::Mat rgb; 
  cv::cvtColor(lab, rgb, cv::COLOR_Lab2BGR);
  rgb.convertTo(img, CV_8UC3, 255.0);
}
void Pix::GetRegionImage(cv::Mat& img) {
  cv::Mat temp;
  cvtColor(input_img_, temp, cv::COLOR_Lab2BGR);
  temp.convertTo(img, CV_8UC3, 255.0);
  for(int y = 0; y< input_height_; ++y) {
    for(int x = 0; x<input_width_; ++x) {
      cv::Vec2i cluster = m_region_map.at<cv::Vec2i>(y,x);
      if(x+1 < m_region_map.cols) {
        cv::Vec2i c2 = m_region_map.at<cv::Vec2i>(y,x+1);
        if (c2[0] != cluster[0] || c2[1] != cluster[1]) {
          img.at<cv::Vec3b>(y,x) = cv::Vec3b(0,0,255);
        }
      }
      if(y+1 < m_region_map.rows) {
        cv::Vec2i c2 = m_region_map.at<cv::Vec2i>(y+1,x);
        if (c2[0] != cluster[0] || c2[1] != cluster[1]) {
          img.at<cv::Vec3b>(y,x) = cv::Vec3b(0,0,255);
        }
      }
...
void Pix::UpdateSuperpixelMapping() {
      ...
      for(int yy = min_y; yy<= max_y; ++yy) {
        for(int xx = min_x; xx<=max_x; ++xx) {
...
#else
          cv::Vec3f cdiff = pixel_color - superpixel_color;
          float color_error = sqrt(cdiff(0)*cdiff(0)+cdiff(1)*cdiff(1)+cdiff(2)*cdiff(2)) ;
          float dist_err = (float)sqrt( (xx-pos(0))*(xx-pos(0))+(yy-pos(1))*(yy-pos(1)) );
#endif
          ...
          if(distance.at<float>(yy,xx) < 0 || error < distance.at<float>(yy,xx)) {
            ...
            m_region_map.at<cv::Vec2i>(yy,xx) = cv::Vec2i(x,y);
          }
        }
      }
    }
  ...
  for(int y = 0; y< input_height_; ++y) {
    for(int x = 0; x<input_width_; ++x) {
      cv::Vec2i superpixel = m_region_map.at<cv::Vec2i>(y,x);
...
int Pix::ExpandPalette()
{
  if(palette_maxed_flag_) return 0;
