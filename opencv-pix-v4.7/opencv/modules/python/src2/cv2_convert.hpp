    ...
    < T, typename std::enable_if< std::is_same<unsigned int, T>::value && !std::is_same<unsigned int, size_t>::value >::type >
{
    ...
    static inline PyObject* from(const T& value)
    {
        return PyLong_FromUnsignedLong(value);
    }
    static inline bool to(PyObject* obj, T& value, const ArgInfo& info)
    {
        CV_UNUSED(info);
        if(!obj || obj == Py_None)
            return true;
        if(PyInt_Check(obj))
            value = (unsigned long)PyInt_AsLong(obj);
        else if(PyLong_Check(obj))
            value = (unsigned long)PyLong_AsLong(obj);
        else
            return false;
        return value != (unsigned long)-1 || !PyErr_Occurred();
    }
...
template<typename T>
struct PyOpenCV_Converter
    < T, typename std::enable_if< std::is_same<long long unsigned, T>::value >::type >
{
    static inline PyObject* from(const T& value)
    {
        return PyLong_FromUnsignedLong(value);
